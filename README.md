...
当前主线版本为 **0.9.1**：WebDAV 录像与元数据在上传前使用分块 AES-256-GCM
...
正常停止、关机或重启的信号会触发最后一批上传，具体等待时间见下方“退出时上传”。
突然断电或强制终止时，尚未提交的 RAM 数据仍会丢失。无上传积压时，当前批次的风险窗口约为：
...
等待正在上传、排队和最后一批媒体及声音索引完成提交。WebDAV 继续使用原有加密和
PUT/MOVE 流程；不需要等到动态 `n` 分钟，也不增加后台监控进程或视频编解码。
FFmpeg 通过专用管道收到 `q` 正常退出指令，避免重复停止信号中断最后一个 HTTP 分片；
只有无响应时才升级为终止信号。

```toml
[server]
shutdown_timeout_seconds = 120 # 停摄像头 + 上传的总等待时间
```

短暂失败会在退出时限内重试；超时日志明确报告未提交字节数，避免把释放 RAM 误报成
上传成功。系统服务管理器必须提供足够等待时间，并让 AList、网络晚于 CamVault 停止。
...
0.9.1 当前回归结果见测试报告；正式 N5105 部署还验证了两路加密上传、密文
