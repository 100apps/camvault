...
当前主线版本为 **0.10.0**：WebDAV 录像与元数据先加密写入持久磁盘队列，再上传网盘；
AList 故障、上传中断或程序重启后自动补传，旧配置不新增参数也默认启用。
录像与元数据使用分块 AES-256-GCM
加密，并能依据每路实时码率和系统可用内存自动调整归档时长及目标大小。历史回放和
按时间导出仍由 CamVault 透明完成，既有明文录像保持可读。正常停止信号会触发剩余录像上传。
...
- 可选两种归档后端：
  ...
  - `webdav`：内存聚合后加密落盘，后台流式 PUT 到 AList/WebDAV；视频及声音索引都提交成功后才删除本地副本。
...
└───────────────────────────┬─────────────────────────────────┘
                  ...
                  ▼                   ▼
            local 后端           webdav 后端
       .partial -> rename     AES-GCM -> 磁盘队列 -> PUT/MOVE
                  │                   │
                  ▼                   ▼
...
| `write_failure_policy` | `delete_oldest` 在本地写失败或 WebDAV 明确 HTTP 507 容量失败时回收最旧录像；`retry` 只重试 |
...
接收一个新 HTTP 分片时还有瞬时内存。磁盘队列不会整批读回 RAM：后台每次读取 128 KiB
密文并上传，只运行一个补传任务；加密/落盘在工作线程完成，不增加视频编解码。

### 断网/网盘故障

WebDAV 模式始终有磁盘兜底，不需要配置开关。每个自适应批次先顺序写入加密视频、
加密声音索引和最小恢复清单，`fsync` 后原子封口，再由后台上传。
只有视频与索引均提交成功才移除本地副本；上传到一半失败也保留整对文件。
启动时扫描未完成队列，AList 离线仍可启动并继续录制；恢复后按时间补传。
首次失败默认等 30 秒重试，连续失败指数退避，最多 300 秒，不会反复轰炸网盘。
待补传录像可直接从磁盘透明回放及导出。
...
```toml
[storage]
# 默认位于配置文件同目录的 spool 子目录；路由器即 /data/camvault/spool。
# 也可以通过 CAMVAULT_SPOOL_DIRECTORY 覆盖默认路径，显式配置优先。
spool_directory = "./spool"
spool_max_gb = 10             # 整个队列上限，GiB，所有摄像头合计
spool_min_free_gb = 1        # 至少保留的磁盘空闲空间，GiB
spool_retry_seconds = 30
spool_upload_timeout_seconds = 60  # 单批补传总超时；慢上行可调大
```

目录必须放在已挂载的持久磁盘，不要放 `/tmp`、tmpfs 或只读系统分区。
默认限制不是可无限保存的承诺：磁盘队列满、剩余空间不足或写入失败时，会报警并保留
尚未上传的录像；RAM 随后达到硬限会反压摄像头，持续故障仍可能造成新录像缺口。
不会为了腾空间删除未上传录像，也不会因本地磁盘满、认证失败或断网而删除云端历史。
WebDAV `delete_oldest` 仅在明确的 HTTP 507 容量不足响应时触发最旧云端录像回收，
受配置的删除量和文件数限制；本地后端保留原有写失败回收策略。

队列目录权限为 0700、文件为 0600；加密启用时视频和完整声音索引落盘即为密文。
恢复清单不保存密码/密钥，但时间、大小、摄像头 ID 和紧凑声音标记仍可见。
目录绑定网盘地址、账户、归档路径和加密密钥，改变这些配置需要先处理旧队列并使用新的
`spool_directory`，避免将旧录像误传给其他账户。密钥仍需独立备份。

正常停止、关机或重启时先把最后一批保存到磁盘，再在退出时限内尝试上传；超时留待
下次启动，具体等待时间见下方“退出时上传”。突然断电或强制终止时，已经完成持久化的
批次可恢复，**尚未封口的 RAM 批次仍可能丢失**。无落盘积压时，风险窗口约为：

```text
min(当前目标秒数, 当前目标字节 / 实际字节率)
```
...
4. 本地归档首次写失败，或 WebDAV 明确 HTTP 507 容量失败时，额外回收受限数量的最旧录像，再重试同一事务；
...
`camvault serve` 收到 `SIGTERM`（正常关机、重启、服务停止）或 `SIGINT`（Ctrl+C）后，
保持分片接收接口开放，先停止摄像头并接收最后一个分片，再封存所有未满批次的录像，
先将排队和最后一批媒体及声音索引加密持久化，再等待 WebDAV 的 PUT/MOVE 提交。
不需要等到动态 `n` 分钟，也不增加后台监控进程或视频编解码。
FFmpeg 通过专用管道收到 `q` 正常退出指令，避免重复停止信号中断最后一个 HTTP 分片；
只有无响应时才升级为终止信号。

```toml
[server]
shutdown_timeout_seconds = 120 # 停摄像头 + 持久化 + 上传的总等待时间
```

短暂失败会在退出时限内重试；超时后，已落盘的批次保留待补传，并单独报告无法落盘的
RAM 字节数，不把暂存成功误报成网盘上传成功。系统服务管理器必须提供足够等待时间，
并让 AList、网络晚于 CamVault 停止。
**本项目 OpenWrt 部署使用 10 秒**，配套 init 脚本在 `K09` 提前停止并等待进程退出，
早于 `K10alist`。OpenWrt 自身的关机脚本时限很短，不能仅靠配置 120 秒延长系统关机窗口；
详见 [`deploy/openwrt/README.md`](deploy/openwrt/README.md)。

上传能否全部完成仍取决于网速、剩余录像大小及网盘状态。突然断电、强制关机或
`SIGKILL` 无法触发退出上传；Windows 计划任务强制结束也不等价于正常停止信号。
...
0.10.0 当前回归结果见测试报告；正式 N5105 部署还验证了两路加密上传、密文
...
- 默认加密磁盘兜底、离线启动、进程强杀后恢复、视频/索引部分提交重试及磁盘满保护；
