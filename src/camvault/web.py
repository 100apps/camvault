        ...
        for camera in service.config.cameras:
            cards.append(
                ...
                <article class="camera-card">
                  <div class="camera-title"><span class="dot" id="dot-{camera.id}"></span>
                  ...
                  <a class="button" href="/player/{camera.id}{token_query}">打开播放器</a>
                </article>
                """
            )
...
main{{max-width:1180px;margin:0 auto;padding:32px 20px 48px}}h1,h2{{margin:0}}.top{{display:flex;justify-content:space-between;align-items:start;gap:16px;margin-bottom:24px}}.lead,small,.muted{{color:var(--muted)}}.lead{{margin:7px 0 0}}
...
.grid{{grid-template-columns:repeat(auto-fit,minmax(240px,1fr));margin-bottom:28px}}.camera-card{{display:grid;gap:12px}}.camera-title{{display:flex;align-items:center}}.dot{{display:inline-block;width:10px;height:10px;border-radius:50%;background:var(--muted);margin-right:8px}}
...
footer{{color:var(--muted);margin-top:28px;font-size:13px}}@media(max-width:780px){{.metrics,.tools{{grid-template-columns:1fr}}.top{{align-items:flex-start;flex-direction:column}}}}
...
<section class="grid">{"".join(cards)}</section><section class="tools"><article class="panel"><h2>配置文件</h2><textarea id="config" spellcheck="false" aria-label="CamVault TOML configuration"></textarea><div class="actions"><button id="save">校验并保存</button><button class="secondary" id="reload">重新读取</button><span id="notice">保存后需重启服务生效</span></div></article>
...
<script>
const token={token_json};let revision=null,lastLog=0;
...
document.getElementById('save').onclick=saveConfig;document.getElementById('reload').onclick=loadConfig;document.getElementById('refreshLogs').onclick=loadLogs;document.getElementById('cleanup').onclick=cleanup;refresh();loadConfig();loadLogs();setInterval(refresh,3000);setInterval(loadLogs,10000);
</script>
</body></html>"""
